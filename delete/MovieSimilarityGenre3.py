...
movie_genre_similarity3 = {}
 
for i in range (20000, 30000):
    if dict.get(str(i), -1) != -1:
        print(i)
        movie_genre_similarity3[str(i)] = {}
        for k in range (i+1, 193887):
            if dict.get(str(k), -1) != -1:
                ...
                if score != 0:
                    ...
                    movie_genre_similarity3[str(i)].update({str(k):score})
                    if movie_genre_similarity3.get(str(k), -1) == -1:
                        movie_genre_similarity3[str(k)] = {}
                    movie_genre_similarity3[str(k)].update({str(k):score})
    


print(movie_genre_similarity3['1'])


output = json.dumps(movie_genre_similarity3)
