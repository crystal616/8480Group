...
movie_genre_similarity2 = {}
 
for i in range (10000, 20000):
    if dict.get(str(i), -1) != -1:
        print(i)
        movie_genre_similarity2[str(i)] = {}
        for k in range (i+1, 193887):
            if dict.get(str(k), -1) != -1:
                ...
                if score != 0:
                    ...
                    movie_genre_similarity2[str(i)].update({str(k):score})
                    if movie_genre_similarity2.get(str(k), -1) == -1:
                        movie_genre_similarity2[str(k)] = {}
                    movie_genre_similarity2[str(k)].update({str(k):score})
    


print(movie_genre_similarity2['1'])


output = json.dumps(movie_genre_similarity2)
f = open("movie_genre_similarity2.json","w")
f.write(output)
f.close()